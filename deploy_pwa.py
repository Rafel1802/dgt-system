import os
import subprocess
import sys

askpass_path = "/Applications/XAMPP/xamppfiles/htdocs/dgt-system/askpass.sh"
with open(askpass_path, "w") as f:
    f.write("#!/bin/bash\necho 'Digital@PhnomPenh#!2027'\n")
os.chmod(askpass_path, 0o755)

env = os.environ.copy()
env["SSH_ASKPASS"] = askpass_path
env["DISPLAY"] = "dummy"
env["SSH_ASKPASS_REQUIRE"] = "force"

def run_cmd(args):
    print(f"Running: {' '.join(args)}")
    proc = subprocess.run(args, env=env, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    print(proc.stdout)
    if proc.returncode != 0:
        print(f"Command failed with exit code {proc.returncode}")
        sys.exit(1)
    print("--- Done ---\n")

# 1. Upload using relative paths (to preserve resources/views/...)
rsync_cmd = [
    "rsync", "-avz", "-e", "ssh -o StrictHostKeyChecking=no -p 65002", "--relative",
    "resources/views/layouts/app.blade.php",
    "resources/views/layouts/auth.blade.php",
    "resources/views/downloads/mac-app.blade.php",
    "resources/views/notes/team.blade.php",
    "resources/views/notes/private.blade.php",
    "resources/views/admin/labels/index.blade.php",
    "resources/views/admin/emails/index.blade.php",
    "resources/views/social-media/table.blade.php",
    "public/manifest.json",
    "app/Services/BoardWorkflowService.php",
    "app/Models/Workspace.php",
    "app/Models/Setting.php",
    "app/Http/Controllers/Board/BoardController.php",
    "app/Http/Controllers/Board/CardController.php",
    "u355625773@157.173.215.124:domains/lightcyan-weasel-711536.hostingersite.com/public_html/"
]
run_cmd(rsync_cmd)

# 2. Upload manifest directly to public_html/ just in case the document root doesn't use the public/ prefix
rsync_public_cmd = [
    "rsync", "-avz", "-e", "ssh -o StrictHostKeyChecking=no -p 65002",
    "public/manifest.json",
    "u355625773@157.173.215.124:domains/lightcyan-weasel-711536.hostingersite.com/public_html/"
]
run_cmd(rsync_public_cmd)

# 3. Clear cache
ssh_cmd = [
    "ssh", "-o", "StrictHostKeyChecking=no", "-p", "65002", "u355625773@157.173.215.124",
    "cd domains/lightcyan-weasel-711536.hostingersite.com/public_html && rm -rf bootstrap/cache/*.php storage/framework/cache/data/* storage/framework/views/* || true"
]
run_cmd(ssh_cmd)

print("Deploy successful!")
