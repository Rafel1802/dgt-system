...
<div x-data="approvalQueue()" class="animate-fade-in pb-28 md:pb-8">
  ...
  @if($pendingCards->isEmpty())
    <div class="card text-center py-20">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="#cbd5e1" class="w-16 h-16 mx-auto mb-4">
        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/>
      </svg>
      <h3 class="font-display font-bold text-slate-700 text-lg mb-2">All Clear! 🎉</h3>
      <p class="text-slate-400 text-sm">No tasks are pending your review right now.</p>
      <a href="{{ route('boards.workspaces') }}" class="btn btn-primary mt-4">View Boards</a>
    </div>
  @else
    <div class="card p-0 overflow-hidden">
      <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
        <h3 class="font-display font-semibold text-slate-800">Tasks Awaiting Review</h3>
        <span class="badge badge-amber">{{ $pendingCards->count() }} pending</span>
      </div>
      ...
      <div class="divide-y divide-slate-100">
        ...
        @php
          ...
          $isSoon    = $card->deadline && !$isOverdue && $card->deadline->diffInDays(now()) <= 2;
          $progress  = $card->checklistProgress();
        @endphp
        <div class="px-6 py-5 hover:bg-slate-50 transition-colors" id="approval-row-{{ $card->id }}">
          <div class="flex items-start gap-4">
            <div class="w-1 self-stretch rounded-full flex-shrink-0" style="background: {{ $card->label_color }}"></div>
            <div class="flex-1 min-w-0">
              <div class="flex items-center flex-wrap gap-2 mb-1">
                @php $p = $card->priority?->value ?? $card->priority ?? 'medium'; @endphp
                <span class="badge {{ match($p) { 'urgent'=>'badge-rose','high'=>'badge-amber','medium'=>'badge-sky',default=>'badge-slate' } }}">
                  {{ ucfirst($p) }}
                </span>
                <span class="badge" style="background:{{ $card->label_bg }};color:{{ $card->label_color }}">
                  {{ $card->label }}{{ $card->sub_label ? ' → '.$card->sub_label : '' }}
                </span>
                @if($card->board)
                <span class="badge badge-slate text-[10px]">📋 {{ $card->board->name }}</span>
                @endif
                @if($isOverdue)
              ...
              <h4 class="font-semibold text-slate-800 text-sm mb-1">{{ $card->title }}</h4>

              @if($card->description)
                <p class="text-slate-400 text-xs line-clamp-2 mb-2">{{ Str::limit($card->description, 120) }}</p>
              @endif

              <div class="flex items-center flex-wrap gap-3 text-xs text-slate-400">
                <span>Created by <span class="font-medium text-slate-600">{{ $card->creator?->name }}</span></span>
                <span>Submitted {{ $card->updated_at->diffForHumans() }}</span>
                @if($card->deadline)
                  <span class="{{ $isOverdue ? 'text-red-500 font-semibold' : '' }}">
                    📅 {{ $card->deadline->format('d M Y') }}
                  </span>
                @endif
                @if($progress['total'] > 0)
                ...
                @endif
                @if($card->files->count())
                  <span>📎 {{ $card->files->count() }} file(s)</span>
                @endif
              </div>

              @if($card->assignees->isNotEmpty())
              <div class="flex items-center gap-1.5 mt-2">
                <span class="text-xs text-slate-400">Assignees:</span>
                @foreach($card->assignees as $a)
                  <img src="{{ $a->avatar_url }}" alt="{{ $a->name }}" class="avatar" style="width:22px;height:22px;" title="{{ $a->name }}">
                @endforeach
              </div>
              @endif
            </div>
            ...
            <div class="flex flex-col gap-2 flex-shrink-0">
              @if($card->board)
                <a href="{{ route('boards.show', $card->board->slug) }}?card={{ $card->id }}"
                   class="btn btn-secondary text-xs py-1.5 px-3" target="_blank">View →</a>
              @else
                <a href="{{ route('boards.workspaces') }}"
                   class="btn btn-secondary text-xs py-1.5 px-3" target="_blank">View →</a>
              @endif
              <button @click="openApprove({{ $card->id }}, '{{ addslashes($card->title) }}')"
                      class="btn text-xs py-1.5 px-3 text-white font-semibold"
                      style="background:linear-gradient(135deg,#10b981,#059669);box-shadow:0 2px 8px rgba(16,185,129,.3)">
                ✓ Approve
              </button>
              <button @click="openReject({{ $card->id }}, '{{ addslashes($card->title) }}')"
  ...
  {{-- ── Approve Modal ───────────────────────────────────────────────────────── --}}
  <div x-show="showApprove" x-cloak class="modal-overlay" @keydown.escape.window="showApprove=false">
    <div class="modal-box max-w-md" @click.stop>
      <div class="modal-header">
        <div>
          <h3 class="font-display font-bold text-slate-800">Approve Task</h3>
          <p class="text-slate-400 text-sm mt-0.5" x-text="`&quot;${activeTitle}&quot;`"></p>
        </div>
        <button @click="showApprove=false" class="btn btn-secondary btn-icon ml-auto">
      ...
      <div class="p-6">
        <div class="flex items-start gap-3 mb-5 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
          <span class="text-2xl">✅</span>
          <div>
            <p class="font-semibold text-emerald-800 text-sm">Confirm Approval</p>
            <p class="text-emerald-600 text-xs mt-0.5">
              This will mark the task as <strong>Approved</strong> and automatically send an email
              notification to all <strong>Boss</strong> users via Google Workspace SMTP.
              The creator will also be notified.
            </p>
          </div>
        </div>
        <div class="flex gap-3">
          <button @click="showApprove=false" class="btn btn-secondary flex-1">Cancel</button>
          <button @click="submitApprove()" :disabled="loading" class="btn text-white flex-1 font-semibold"
                  style="background:linear-gradient(135deg,#10b981,#059669)">
            <span x-show="!loading">✓ Approve & Notify Boss</span>
            <span x-show="loading" x-cloak>Sending email…</span>
          </button>
        </div>
      </div>
    </div>
  ...
  {{-- ── Reject Modal ────────────────────────────────────────────────────────── --}}
  <div x-show="showReject" x-cloak class="modal-overlay" @keydown.escape.window="showReject=false">
    <div class="modal-box max-w-md" @click.stop>
      <div class="modal-header">
        <div>
          <h3 class="font-display font-bold text-slate-800">Return for Revision</h3>
          <p class="text-slate-400 text-sm mt-0.5" x-text="`&quot;${activeTitle}&quot;`"></p>
        </div>
        <button @click="showReject=false" class="btn btn-secondary btn-icon ml-auto">
          <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12"/></svg>
        </button>
      </div>
      <div class="p-6 space-y-4">
        <div class="p-4 bg-rose-50 border border-rose-200 rounded-xl text-sm text-rose-700">
          ❌ The task will be returned to the creator with your feedback. They will receive an email explaining what needs to be changed.
        </div>
        <div>
          <label class="form-label">Reason / Feedback <span class="text-red-500">*</span></label>
          <textarea x-model="rejectReason" rows="4" class="form-input"
                    placeholder="Explain clearly what needs to be changed or improved…"></textarea>
          <p class="text-xs text-slate-400 mt-1">This message will be sent directly to the creator via email.</p>
        </div>
        <div class="flex gap-3">
          <button @click="showReject=false; rejectReason=''" class="btn btn-secondary flex-1">Cancel</button>
          <button @click="submitReject()" :disabled="loading || !rejectReason.trim()" class="btn btn-danger flex-1">
            <span x-show="!loading">✗ Send Back for Revision</span>
