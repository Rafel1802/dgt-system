...
<div class="animate-fade-in">
  ...
  <div class="card p-0 overflow-hidden" x-data="{
    selected: [],
    get allChecked() { return {{ $callReports->count() }} > 0 && this.selected.length === {{ $callReports->count() }}; },
    toggleAll(e) { this.selected = e.target.checked ? {{ Js::from($callReports->pluck('id')) }} : []; }
  }">
    {{-- Bulk Action Bar --}}
    <div x-show="selected.length > 0" x-cloak class="flex items-center justify-between bg-slate-900 text-white px-5 py-3 border-b border-slate-800">
      <div class="flex items-center gap-2 text-sm">
        ...
        <span class="font-medium" x-text="selected.length + ' report(s) selected'"></span>
      </div>
      <div class="flex items-center gap-2">
        <form method="POST" action="{{ route('crm.website.call-reports.bulk-destroy') }}"
              onsubmit="return confirm('Are you sure you want to delete the selected call report(s)?')" class="inline">
          @csrf
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-50">
          ...
          <tr class="hover:bg-slate-50/70 transition-colors" :class="selected.includes({{ $report->id }}) ? 'bg-indigo-50/40' : ''">
            <td class="px-5 py-3 w-10">
              <input type="checkbox" :value="{{ $report->id }}" x-model="selected" class="accent-indigo-600 w-4 h-4 rounded cursor-pointer">
            </td>
            <td class="px-4 py-3 font-semibold text-slate-800">{{ $report->name ?: '—' }}</td>
