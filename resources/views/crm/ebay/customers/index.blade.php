...
<div id="crm-ebay-customers-list" class="animate-fade-in live-swap-target">
  ...
  <div class="card p-0 overflow-hidden" x-data="{
    selected: [],
    get allChecked() { return {{ $records->count() }} > 0 && this.selected.length === {{ $records->count() }}; },
    toggleAll(e) { this.selected = e.target.checked ? {{ Js::from($records->pluck('id')) }} : []; }
  }">
    {{-- Bulk Action Bar --}}
    <div x-show="selected.length > 0" x-cloak class="flex items-center justify-between bg-slate-900 text-white px-5 py-3 border-b border-slate-800">
      <div class="flex items-center gap-2 text-sm">
        ...
        <span class="font-medium" x-text="selected.length + ' eBay record(s) selected'"></span>
      </div>
      <div class="flex items-center gap-2">
