  ...
  <div class="card">
    ...
    <form method="POST" action="{{ route('crm.ebay.customers.store', $tab) }}" class="divide-y divide-slate-100">
      ...
      <div class="px-6 py-5 space-y-4">
        ...
        <input id="field-username" type="hidden" name="username" value="{{ old('username') }}">
        @endif

        @if(in_array('buyer_name', $columns))
        <input id="field-buyer-name" type="hidden" name="buyer_name" value="{{ old('buyer_name') }}">
        @endif

        @if(in_array('email', $columns))
        <input id="field-email" type="hidden" name="email" value="{{ old('email') }}">
        @endif
