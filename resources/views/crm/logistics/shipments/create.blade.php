    ...
    <form method="POST" action="{{ route('crm.logistics.shipments.store') }}" class="divide-y divide-slate-100">
      ...
      <div class="px-6 py-5 space-y-4">
        <div>
          ...
          <input type="text" name="shipment_code" value="{{ old('shipment_code', 'SHP-'.date('Ymd-Hi')) }}" class="form-input font-mono @error('shipment_code') error @enderror" required>
          @error('shipment_code')<p class="form-error">{{ $message }}</p>@enderror
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label class="form-label">Estimated Arrival Date <span class="text-red-500">*</span></label>
            <input type="date" name="estimated_arrival" value="{{ old('estimated_arrival', date('Y-m-d')) }}" class="form-input" required>
          </div>
          <div>
        ...
        <div>
          ...
          <textarea name="notes" rows="3" class="form-input" placeholder="General instructions for this shipment group...">{{ old('notes') }}</textarea>
        </div>
      </div>
