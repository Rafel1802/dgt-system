...
@section('title', 'Create Popup Ad')
@section('page_title', 'Create Popup Ad')
@section('content')
@section('back_url', route('admin.popup-ads.index'))
<div class="max-w-4xl mx-auto pb-12">
    <div class="flex items-center gap-4 mb-8">
        <h1 class="text-3xl font-black text-slate-900 dark:text-white">Create Popup Ad</h1>
    </div>

    <form action="{{ route('admin.popup-ads.store') }}" method="POST" enctype="multipart/form-data" class="bento-card p-6 sm:p-8 space-y-6">
        @csrf
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Title (Internal)</label>
                <input type="text" name="title" value="{{ old('title') }}" required class="w-full form-input bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl">
                @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Image (Optional)</label>
                <div x-data="imageUploader()" class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl dark:border-slate-700 hover:border-indigo-500 transition-colors bg-slate-50 dark:bg-slate-800/50" 
            ...
            <div>
                ...
                <input type="number" name="interval_minutes" value="{{ old('interval_minutes', 5) }}" required min="1" class="w-full form-input bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="body_text" rows="3" class="w-full form-input bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl">{{ old('body_text') }}</textarea>
            ...
            <div>
                ...
                <input type="text" name="button_text" value="{{ old('button_text', 'Click Here') }}" class="w-full form-input bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl">
            ...
            <div>
                ...
                <input type="url" name="button_link" value="{{ old('button_link') }}" placeholder="https://" class="w-full form-input bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl">
            ...
            <div>
                ...
                <input type="text" name="notification_text" value="{{ old('notification_text', 'Special Offer!') }}" class="w-full form-input bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl">
            ...
            <div>
                ...
                <input type="text" name="notification_icon" value="{{ old('notification_icon', '🎁') }}" class="w-full form-input bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl">
            ...
            <div>
                ...
                <input type="datetime-local" name="start_time" value="{{ old('start_time') }}" class="w-full form-input bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl">
                @error('start_time') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">End Date/Time (Optional)</label>
                <input type="datetime-local" name="end_time" value="{{ old('end_time') }}" class="w-full form-input bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl">
