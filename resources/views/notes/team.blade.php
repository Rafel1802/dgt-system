...
<style>
    ...
    @media (max-width: 1023px) {
        ...
        /* On mobile the 3 columns become stacked panels controlled by mobile-panel data */
        ...
        /* Panel 0 = folders, 1 = note list, 2 = editor */
        ...
        /* Editor padding reduction on mobile */
        .notes-editor-card .ql-editor {
...
<div class="bg-slate-50 flex notes-shell font-sans"
     x-data="notesApp('team', '{{ $currentTeam }}')"
     x-init="initApp()"
    ...
    <div class="notes-col-folder w-64 bg-slate-100 border-r border-slate-200 flex flex-col flex-shrink-0"
        ...
        <div class="lg:hidden flex items-center justify-between px-4 py-3 border-b border-slate-200 bg-white">
            <span class="text-sm font-black text-slate-700">Team Notes</span>
        ...
        <div class="p-4 pt-6">
            ...
            <div class="mb-4 px-2">
                <form action="{{ route('notes.team') }}" method="GET">
                    <select name="team" onchange="this.form.submit()" class="w-full bg-white border-slate-200 text-sm rounded-lg shadow-sm focus:ring-yellow-400 focus:border-yellow-400">
                        @foreach($accessibleTeams as $t)
                            <option value="{{ $t }}" {{ $currentTeam === $t ? 'selected' : '' }}>{{ ucfirst($t) }} Team</option>
                        @endforeach
                    </select>
                </form>
            </div>
    ...
    <div class="notes-col-list w-80 bg-white border-r border-slate-200 flex flex-col flex-shrink-0"
        ...
        <div class="p-3">
            <div class="relative">
                ...
                <input type="text" x-model="searchQuery" @input.debounce.300ms="fetchNotes()" placeholder="Search team notes" class="w-full pl-9 pr-3 py-2 bg-slate-100 border-transparent rounded-lg text-sm focus:bg-white focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400/20 transition-all">
            </div>
    ...
    <div x-show="showFolderModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm" x-cloak x-transition.opacity>
        <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4" @click.away="showFolderModal = false" x-transition>
