      ...
      <div class="px-6 py-3 border-b border-slate-100 bg-white flex items-center justify-between gap-3 flex-wrap">
        <div class="flex items-center gap-3">
          <label class="flex items-center gap-2 cursor-pointer select-none group">
            <input type="checkbox" @change="toggleSelectAll($event.target.checked)" :checked="selectedCount === {{ $totalTrashed }}" :indeterminate="selectedCount > 0 && selectedCount < {{ $totalTrashed }}"
...
async function boardQuickAction(action, slug, name) {
...
function trashManager() {
  return {
    ...
    toggleSelectAll(checked) {
      if (checked) {
        this.selected.workspaces = [{{ isset($trashedWorkspaces) ? $trashedWorkspaces->pluck('id')->implode(',') : '' }}];
        this.selected.boards = [{{ isset($trashedBoards) ? $trashedBoards->pluck('id')->implode(',') : '' }}];
      } else {
