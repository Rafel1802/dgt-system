...
<div x-show="memberPicker.open" x-cloak
  ...
  <div class="relative bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-md overflow-hidden"
    ...
    <div x-show="!memberPicker.loading" class="max-h-[30rem] overflow-y-auto pb-3 scrollbar-thin">

      {{-- ── Card Members ─────────────────────────────── --}}
      <template x-if="memberPicker.cardMembers.length">
        <div>
          <p class="text-xs uppercase font-extrabold text-slate-400 px-4 pt-4 pb-2 select-none tracking-wider">
            Card Members
          </p>
          <template x-for="u in memberPicker.cardMembers" :key="'cm-'+u.id">
            <button @click="mpToggleMember(u)"
                    class="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-indigo-50/50 transition-colors group">
              {{-- Avatar --}}
              ...
              {{-- Check mark --}}
              <svg class="w-5 h-5 text-indigo-600 flex-shrink-0" fill="none" viewBox="0 0 24 24"
        ...
        <div>
          ...
          <template x-for="u in memberPicker.boardMembers" :key="'bm-'+u.id">
            <button @click="mpToggleMember(u)"
              ...
              {{-- Plus icon to add --}}
              <svg class="w-5 h-5 text-slate-300 group-hover:text-indigo-500 flex-shrink-0 transition-colors"
                   fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
